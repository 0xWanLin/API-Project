...
def search(request):
    ...
    # a_query = DomainIpScan.objects.filter(id=query)
    # b_query = CommunicatingFile.objects.filter(id=query)
    # c_query = ReferringFile.objects.filter(id=query)

    # return render(request, 'search_domain_ip.html', {'a_query': a_query, 'b_query': b_query, 'c_query': c_query})
...
def search_file(request):
    ...
    # z_query = FileScan.objects.filter(file_id=query)
    # x_query = ExecutionParent.objects.filter(file_id=query)
    # return render(request, 'search_file.html', {'z_query': z_query, 'x_query': x_query})
